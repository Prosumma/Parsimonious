...
public func opt<T>(parser: ParseContext -> ParseResult<T>) -> (ParseContext -> ParseResult<T>) {
    return { context in
...
public func not<T>(parser: ParseContext -> ParseResult<T>) -> (ParseContext -> ParseResult<T>) {
    return { context in
...
public func many<T>(parser: ParseContext -> ParseResult<T>) -> (ParseContext -> ParseResult<T>) {
    return { context in
        var matches = Array<ParseResult<T>.Match>()
        parsing: while true {
            switch parser(context) {
            ...
            case let .Error(error, index): return .Error(error, index)
            }
        }
        return .Matched(matches)
    }
...
public func skip<T1, T2>(parser: ParseContext -> ParseResult<T1>) -> (ParseContext -> ParseResult<T2>) {
    return { context in
        switch parser(context) {
...
public func some<T>(range: Range<UInt>) -> ((ParseContext -> ParseResult<T>) -> (ParseContext -> ParseResult<T>)) {
    return { parser in
        return { context in
            let position = context.position
            let parseResult = parser(context)
...
public func expect<T>(parser: ParseContext -> ParseResult<T>, error: ErrorType) -> (ParseContext -> ParseResult<T>) {
    return { context in
        let parseResult = parser(context)
        switch parseResult {
...
public func lift<T1, T2>(parser: ParseContext -> ParseResult<T1>, transform: [(T1, Range<String.Index>)] -> [(T2, Range<String.Index>)]) -> (ParseContext -> ParseResult<T2>) {
    return { context in
        switch parser(context) {
...
public func lift<T1, T2>(parser: ParseContext -> ParseResult<T1>, transform: T1 -> T2) -> (ParseContext -> ParseResult<T2>) {
    return { context in
        switch parser(context) {
...
public func debug<T>(parser: ParseContext -> ParseResult<T>) -> (ParseContext -> ParseResult<T>) {
    return { context in
